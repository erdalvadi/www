...
class Keywords {
	...
	public function extract_addable_track_keyword( $keywords ) {
		...
		// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery -- The $keywords_joined variable used in the query is sanitized.
		$data = $wpdb->get_results( "SELECT keyword FROM {$wpdb->prefix}rank_math_analytics_keyword_manager as km WHERE km.keyword IN ( $keywords_joined )" );
